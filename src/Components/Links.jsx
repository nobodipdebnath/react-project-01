import React from 'react'

function Links() {
  return (
    <div>
        {/* Links Section start */}
        <section>
          <div className="mx-[10%] flex justify-between items-center py-20">
            <img className="cursor-pointer" src="/images/group5.png" alt="" />
            <img className="cursor-pointer" src="/images/group.png" alt="" />
            <img className="cursor-pointer" src="/images/group1.png" alt="" />
            <img className="cursor-pointer" src="/images/Group.png" alt="" />
            <img className="cursor-pointer" src="/images/group2.png" alt="" />
            <img className="cursor-pointer" src="/images/group3.png" alt="" />
            <img className="cursor-pointer" src="/images/group4.png" alt="" />
          </div>
        </section>
        {/* Links section end */}
    </div>
  )
}

export default Links